...
2. **Python 3.10+** с **PyGObject** (Python-биндингами к GTK3). Обычный
   Python с python.org тут не подойдёт — готового PyGObject-колеса для
   Windows на PyPI нет, а собрать его самому без GTK3 и компилятора
   нельзя. Практический путь — [MSYS2](https://www.msys2.org/): это
   отдельная среда со своим пакетным менеджером `pacman` (он **не**
   запускается в обычной cmd/PowerShell — это команда изнутри MSYS2).
   Установите MSYS2, откройте **из его меню Пуск** ярлык «MSYS2 MinGW64»
   (это отдельное окно терминала, не PowerShell) и в нём выполните:
   ```bash
   pacman -S mingw-w64-x86_64-python-gobject mingw-w64-x86_64-gtk3 mingw-w64-x86_64-python-pip
   ```
3. Split VPN — **в том же окне MSYS2 MinGW64**, тем же `pip`:
   ```bash
   ...
   разделения по приложению. Команду `splitvpn` дальше тоже запускайте из
   этого MSYS2-терминала (или добавьте `C:\msys64\mingw64\bin` в PATH) —
   обычный python.org-Python эту установку не увидит.

Первое **Connect** (и каждый **Launch**) вызывает обычный запрос UAC —
так приложение получает разрешение менять маршруты или управлять
фильтром пакетов для отдельных приложений; сам GUI никогда не работает с
повышенными правами.
